...
public static class BasisWebCameraE2EProbe
{
    ...
    private static void Run()
    {
        ...
        probe.AddComponent<Runner>().Initialize(mode);
    }
    ...
    private sealed class Runner : MonoBehaviour
    {
        ...
        private void CapturePanorama(bool exr)
        {
            ...
            var cubemap = new RenderTexture(PanoramaFaceSize, PanoramaFaceSize, 24, RenderTextureFormat.ARGBFloat)
            {
            ...
            var equirect = new RenderTexture(PanoramaWidth, PanoramaHeight, 0, RenderTextureFormat.ARGBFloat)
            {
                antiAliasing = 1,
                useMipMap = false,
                autoGenerateMips = false,
            ...
            {
                byte[] rgba = BasisHandHeldCamera.TonemapEquirectToRgba32(
                    raw, PanoramaWidth, PanoramaHeight, 1f, 1f, 1f);
            ...
            string extension = exr ? "exr" : "png";
            string contentType = exr ? "application/octet-stream" : "image/png";
            BasisWebFileDownload.Save(
                $"Screenshot360_Mono_E2E_{PanoramaWidth}x{PanoramaHeight}.{extension}",
                imageData,
                contentType);
