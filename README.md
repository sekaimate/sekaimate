...
ネイティブクライアントの接続先も`rooms`のドメインです。Agonesが返すノードのアドレスはクラウドではプライベートIPで、
そのままでは招待リンクが届かない宛先になるため、`k3s:up`が`BASIS_SERVER_PUBLIC_HOST`へ`rooms`のドメインを設定し、
Conciergeが会議のホストとして記録します。ポートは会議ごとにAgonesが割り当てた値をそのまま使います。
